...
print('Set Labels ...')
real_data_ben['label'] = [0] * real_data_ben.shape[0]
print(real_data_ben)

real_data_syn['label'] = [1] * real_data_syn.shape[0]
print(real_data_syn)
...
real_data = pd.concat([real_data_ben, real_data_syn], axis=0).sample(frac=1, random_state=rstate)
...
plt.savefig('{}/cf_matrix_synddos_real_trained.svg'.format(output_path), dpi=300)
...
plt.savefig('{}/cf_matrix_synddos_gen_trained.svg'.format(output_path), dpi=300)
