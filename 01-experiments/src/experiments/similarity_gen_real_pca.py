...
gen_data_syn = pd.DataFrame(data=gen_data_syn, columns=['pc_1'])
print(gen_data_syn)
...
print('Concat Data ...')
pca_data = pd.concat([pca_data_ben, pca_data_syn], axis=0)
print(pca_data)
gen_data = pd.concat([gen_data_ben, gen_data_syn], axis=0)
print(gen_data)

print('Label Data ...')
pca_data['label'] = [0] * pca_data.shape[0]
...
plt.savefig('{}/cf_sep_real_pca_syn_data_lsvc.svg'.format(output_path), dpi=300)
plt.show()
...
plt.savefig('{}/cf_sep_real_pca_syn_data_rfc.svg'.format(output_path), dpi=300)
plt.show()